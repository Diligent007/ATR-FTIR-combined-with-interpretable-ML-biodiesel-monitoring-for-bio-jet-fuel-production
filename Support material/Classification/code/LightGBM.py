...
# 划分数据集并记录每个数据的行索引
x_train, x_test, y_train, y_test, indices_train, indices_test = train_test_split(x_data_scaled, y_data, range(len(y_data)), test_size=0.3, random_state=42)
...
accuracy_scores = cross_val_score(model_lgbm, x_train, y_train, cv=cv, scoring='accuracy')
precision_scores = cross_val_score(model_lgbm, x_train, y_train, cv=cv, scoring='precision')
recall_scores = cross_val_score(model_lgbm, x_train, y_train, cv=cv, scoring='recall')
f1_scores = cross_val_score(model_lgbm, x_train, y_train, cv=cv, scoring='f1')
...
# 创建包含真实值和预测值的DataFrame，同时保留原始索引
...
def reorder_and_mix(df):
    # 根据 '真实值' 和 '原始索引' 排序
    df_sorted = df.sort_values(by='真实值').reset_index(drop=True)

    # 保持原始索引和真实值的捆绑关系，并打乱预测值列
    df_sorted['预测值'] = sorted(df_sorted['预测值'])
...
# 计算ROC曲线和AUC
fpr, tpr, _ = roc_curve(y_test_shuffled, model_lgbm.predict_proba(x_test)[:, 1])
roc_auc = auc(fpr, tpr)
...
metrics_results.to_csv('C194-metrics_results.csv', index=False)

roc_curve_data = pd.DataFrame({
    'FPR': fpr,
    'TPR': tpr
})
roc_curve_data.to_csv('C194-roc_curve.csv', index=False)

# 保存混淆矩阵到CSV
conf_matrix_df = pd.DataFrame(conf_matrix, index=['Actual_0', 'Actual_1'], columns=['Predicted_0', 'Predicted_1'])
conf_matrix_df.to_csv('C194-confusion_matrix.csv', index=True)
